...
function obj:moveAndResize(option)
    ...
    if cwin then
        ...
        if option == "halfleft" then
            cwin:setFrame({x=cres.x + 50, y=cres.y, w=cres.w/2 -50, h=cres.h})
        elseif option == "halfright" then
            cwin:setFrame({x=cres.x + 50+cres.w/2, y=cres.y, w=cres.w/2 - 50, h=cres.h})


        -- 定义  lesshalfleft、onethird、lesshalfright
        elseif option == "lesshalfleft" then
            cwin:setFrame({x=cres.x + 50, y=cres.y, w=cres.w/3 - 50, h=cres.h})
        elseif option == "onethird" then
            cwin:setFrame({x=cres.x + 50+cres.w/3, y=cres.y, w=cres.w/3 - 50, h=cres.h})
        elseif option == "lesshalfright" then
            cwin:setFrame({x=cres.x + 50+cres.w/3*2, y=cres.y, w=cres.w/3 - 50, h=cres.h})
        
        -- 定义 mostleft、mostright
        elseif option == "mostleft" then
            cwin:setFrame({x=cres.x + 50, y=cres.y, w=cres.w/3 *2 - 50, h=cres.h})
        elseif option == "mostright" then
            cwin:setFrame({x=cres.x + 50+cres.w/3, y=cres.y, w=cres.w/3 *2 - 50, h=cres.h})
